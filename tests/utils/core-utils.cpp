...
namespace {
    ...
    bool setPathFromEnvironmentVariable(QString *path, QString envVar) {
        ...
        int index = env.indexOf(QRegularExpression('^' + envVar + "=.*"));
...
bool isTestingInSandbox()
{
    ...
    const QStringList env(QProcess::systemEnvironment());
    int index = env.indexOf(QRegularExpression('^' + TestingInSandboxEnvVariable + "=.*"));
    if (index != -1) {
        QString statusCandidate = env.at(index);
        statusCandidate = statusCandidate.remove(
                              QRegularExpression('^' + TestingInSandboxEnvVariable + '='));
        bool statusOk = false;
